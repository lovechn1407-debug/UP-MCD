...
import { Wrench, Plus, CheckCircle2, Loader2, Phone, MapPin } from 'lucide-react';
    ...
    <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">District Field Workers</h2>
            <p className="text-xs text-slate-500">Manage field staff assigned to {userData?.districtId}</p>
          </div>
        </div>
        ...
        >
          ...
          <span>{showForm ? 'Close Form' : 'Register District Worker'}</span>
        </button>
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <h3 className="text-sm font-bold text-slate-900">New Worker Details</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                <label className="text-xs font-semibold text-slate-700 block mb-1">Assigned Ward / Address</label>
                <input
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:border-blue-500 focus:bg-white"
                  value={address}
                  onChange={e => setAddress(e.target.value)}
                  placeholder="e.g. Ward 12, Main Bazaar"
                />
              </div>
            </div>
        ...
        <table className="w-full text-left text-xs text-slate-700">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/70 text-slate-500 font-semibold uppercase tracking-wider">
              ...
              <th className="py-3 px-4">Assigned Ward Address</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan="5" className="py-8 text-center text-slate-400">Loading workers...</td></tr>
            ) : workers.length === 0 ? (
              <tr><td colSpan="5" className="py-8 text-center text-slate-400">No field workers added yet</td></tr>
            ) : workers.map(w => (
